...
> | `flink/normalization/job.py` / `app.py` (`NormalizationFunction`) | Not built yet - `flink/normalization/app.py` today only has a `__main__` smoke-test block reading the local, untracked `event_sample.json` capture directly, no `FlatMapFunction`/Kafka wiring (that's T15/T16 in `tasks.md`, still open) |
