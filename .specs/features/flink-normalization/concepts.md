...
**The refresher**: an ABC (`EventEvaluator`, `flink/normalization/domain/__init__.py`) describes *what* a normalizer does (`apply(event, contract) -> dict`), never *how*. Nothing in the Flink job is allowed to know GitHub-specific details - it only ever talks to the ABC's contract, and per-source knowledge lives entirely in a YAML contract (`AD-006`), not in a class per source.

**The new wrinkle - why this matters more here than it did in ingestion**: in `ingestion/app.py`, you pick the concrete engine by `--source` at the command line, once, at startup. Here, a *single running job* processes events from potentially multiple sources mixed together on the same topic (today only GitHub publishes, but the topic and the ABC don't know that) - the dispatch (`ContractRepository.get(event.source)`, cached) happens **per message**, inside the hot path. This is the same pattern, but exercised differently: not "pick one engine and go," but "look up the right contract, every single time, based on data you just read."
