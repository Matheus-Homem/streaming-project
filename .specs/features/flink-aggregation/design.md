...
graph LR
    ...
    D[analytics<br/>jobmanager, standalone-job -py app.py] --- E[taskmanager-analytics]
    end
```
...
| --- | --- | --- |
| Flink `Dockerfile` pattern (base image + Kafka SQL connector JAR + `usrlib` layout) | `flink/normalization/Dockerfile` | Copy-and-adapt for `flink/analytics/Dockerfile` — the JAR download step is already there and unused by normalization's DataStream job; this feature is its first real consumer |
| `standalone-job -py` jobmanager + dedicated taskmanager compose pattern | `infra/docker/docker-compose.yml` (`normalization`/`taskmanager` services) | Copy-and-adapt into `analytics`/`taskmanager-analytics` |
...
| `shared/logger.py` (`setup_logging`) | `shared/logger.py` | Same per-class logger pattern in `flink/analytics/app.py` |
| `.env` / `dotenv` load + `KAFKA_BOOTSTRAP_SERVERS` env var pattern | `flink/normalization/app.py` | Same shape in `flink/analytics/app.py` |
| Config-selects-behavior pattern (`python -m ingestion.app --source github`) | `ingestion/app.py` | `flink/analytics/app.py` reads *which* query to run from an env var (`ANALYTICS_QUERY_FILE`) instead of hardcoding a filename — same principle, different mechanism (env var, since this runs inside a container without CLI args) |
...
| `interface/sources/github/normalization.yml` (as a DDL source) | `interface/sources/github/normalization.yml` | Considered and rejected during Design (see Tech Decisions) — it has no SQL type information and varies per `event_type`, which the P1 query does not need. `flink/normalization/models.py`'s `NormalizedEvent` (the fixed envelope shape) is the correct reference for the 2 columns P1's source DDL needs, consulted by hand, not read programmatically. |

---

## Components

### `flink/analytics/app.py`

- **Purpose**: Generic Flink SQL script runner. Bootstraps a `StreamExecutionEnvironment`/`StreamTableEnvironment`, configures checkpointing (interval + state backend), reads the `.sql` file named by `ANALYTICS_QUERY_FILE`, splits it into statements, executes each via `execute_sql()`. Never references a specific query by name — a future second query is a new `.sql` file plus a new `docker-compose.yml` service block pointing `ANALYTICS_QUERY_FILE` at it, with zero change to this file.
- **Location**: `flink/analytics/app.py`
- **Interfaces**: none exposed (a `__main__` entrypoint, like `flink/normalization/app.py`)
- **Dependencies**: `KAFKA_BOOTSTRAP_SERVERS`, `ANALYTICS_QUERY_FILE` env vars; `shared.logger.setup_logging`
- **Reuses**: `shared/logger.py`, the `.env`/dotenv-load shape of `flink/normalization/app.py`

### `interface/analytics/repo_counts_5m.sql`
...
- **Location**: `interface/analytics/repo_counts_5m.sql`
- **Not Python** — pure data, per `AD-009`.

### `flink/analytics/Dockerfile`

- **Purpose**: Same base image (`flink:2.3.0-scala_2.12-java17`) and Kafka SQL connector JAR download as `flink/normalization/Dockerfile`. Copies `flink/analytics/`, `shared/`, and `interface/` — does **not** copy `flink/common/` (unused by a Table API job).
- **Location**: `flink/analytics/Dockerfile`
- **Reuses**: `flink/normalization/Dockerfile`'s structure

### `infra/docker/docker-compose.yml` additions

- **`analytics`**: jobmanager, `standalone-job -py flink/analytics/app.py`, `KAFKA_BOOTSTRAP_SERVERS`, `ANALYTICS_QUERY_FILE=repo_counts_5m.sql`, `depends_on: topic-init: condition: service_completed_successfully`
- **`taskmanager-analytics`**: `depends_on: analytics`
...
| --- | --- | --- | --- |
| Exact PyFlink Table API / SQL connector option names not yet verified against Flink 2.3 docs (`json.ignore-parse-errors`, `sink.delivery-guarantee`, `sink.transactional-id-prefix`) | `interface/analytics/repo_counts_5m.sql` (to be written) | Risk of wrong config or a job that silently misbehaves | Knowledge Verification Chain research step during Execute, before finalizing the `.sql`; live docker verification (T6) is the real gate regardless |
| PyFlink `TableEnvironment`'s exact API for running a multi-statement `.sql` file not yet confirmed (per-statement `execute_sql()` loop assumed, not verified) | `flink/analytics/app.py` (to be written) | Same as above | Same as above |
...
| `app.py` shape | Generic SQL-script runner; query file selected via `ANALYTICS_QUERY_FILE` env var, never hardcoded | Mirrors `ingestion/app.py`'s `--source` pattern; a new query never requires a Python change (`AD-006`'s test, applied to queries) |
...
| Source table DDL | Hand-written (envelope + `partition_key` columns only), not derived from `interface/sources/github/normalization.yml` | P1's query never touches `event_types` columns; the contract has no SQL-type vocabulary and would need a union-of-all-types scheme this feature doesn't exercise. Revisit if a future query needs per-type fields. |
